...
abstract class Bip32Ed25519Base(private var seed: ByteArray) {
        ...
        companion object {
                ...
                /**
                 * Harden a number (set the highest bit to 1) Note that the input is UInt and the
                 * output is also UInt
                 *
                 * @param num
                 * @returns
                ...
                /**
                 * Implementation how to validate data with encoding and schema, using base64 as an
                 * example
                 *
                 * @param message
                 * @param metadata
                ...
                /**
                 * Reference of BIP32-Ed25519 Hierarchical Deterministic Keys over a Non-linear
                 * Keyspace
                 *
                 ...
                 * @returns
                 * - Extended root key (kL, kR, c) where kL is the left 32 bytes of the root key, kR
                 * is the right 32 bytes of the root key, and c is the chain code. Total 96 bytes
                 */
         ...
         *
         * @param extendedKey
         * - extended key (kL, kR, c) where kL is the left 32 bytes of the root key the scalar
         * (pvtKey). kR is the right 32 bytes of the root key, and c is the chain code. Total 96
         * bytes
         * @param index
        ...
        public fun deriveChildNodePrivate(
                ...
                val cc =
                                extendedKey.sliceArray(
                                                2 * ED25519_SCALAR_SIZE until
                                                                2 * ED25519_SCALAR_SIZE +
                                                                                CHAIN_CODE_SIZE
                                )
                ...
                var right =
                                (BigInteger(1, kr.reversedArray()) +
                                                                BigInteger(1, zr.reversedArray()))
                                                .toByteArray()
                                                .reversedArray()
                                                .let { bytes ->
                                                        bytes.sliceArray(
         ...
         *
         * @param extendedKey
         * - extend public key (p, c) where p is the public key and c is the chain code. Total 64
         * bytes
         * @param index
         * - unharden index (i < 2^31) of the child key
         * @param g
         * - Defines how many bits to zero in the left 32 bytes of the child key. Standard
         * BIP32-ed25519 derivations use 32 bits.
         * @returns
         * - 64 bytes, being the 32 bytes of the child key (the new public key) followed by the 32
         * bytes of the chain code
         */
        ...
        internal fun trunc256MinusGBits(zl: ByteArray, g: Int): ByteArray {
                if (g < 0 || g > 256) {
                        throw IllegalArgumentException(
                                        "Number of bits to zero must be between 0 and 256."
                        )
                }
         ...
         * @param bip44Path
         * - BIP44 path (m / purpose' / coin_type' / account' / change / address_index). The '
         * indicates that the value is hardened
         * @param isPrivate
         * - returns full 64 bytes privatekey (first 32 bytes scalar), false returns 32 byte public
         * key,
         * @returns
        ...
        public fun deriveKey(
                ...
                if (isPrivate) {
                        ...
                        return lazySodium.cryptoScalarMultEd25519BaseNoclamp(
                                                        derived.sliceArray(
                                                                        0 until ED25519_SCALAR_SIZE
                                                        )
                                        )
                                        .toBytes() +
                                        derived.sliceArray(
         ...
         * @param metadata
         * - metadata object that describes how `data` was encoded and what schema to use to
         * validate against
         *
         * @returns
         ...
         * - key index. This value will be a SOFT derivation as part of BIP44.
         * @param tx
         * - Transaction object containing parameters to be signed, e.g. sender, receiver, amount,
         * fee,
         *
         * @returns stx
                ...
                var S =
                                this.safeModQ(
                                                lazySodium.cryptoCoreEd25519ScalarAdd(
                                                                r,
                                                                lazySodium.cryptoCoreEd25519ScalarMul(
                                                                                                h,
                                                                                                scalar
                                                                                )
                                                                                .toByteArray()
        ...
        fun safeModQ(input: BigInteger): ByteArray {
                var reduced =
                                lazySodium.cryptoCoreEd25519ScalarReduce(input)
                                                .toByteArray()
                                                .reversedArray()
                if (reduced.size < 32) {
         ...
         * encryption and decryption. Creating a private channel between the two parties. Note that
         * you must specify the order of concatenation for the public keys with otherFirst.
         * @param context
         * - context of the key (i.e Address, Identity)
         * @param account
         * - account number. This value will be hardened as part of BIP44
         ...
         * @param meFirst
         * - decide the order of concatenation of the public keys in the shared secret, true: my
         * public key first, false: other party's public key first
         * @returns
        ...
        fun ECDH(
                ...
                val publicKey: ByteArray =
                                this.keyGen(context, account, change, keyIndex, derivationType)
                ...
                lazySodium.convertPublicKeyEd25519ToCurve25519(
                                otherPartyCurve25519Key,
                                otherPartyPub
                )
